...
用 `read_file` 尝试读取 `_personal/{workspace_id}/{staff_id}/{skill_name}/SKILL.md`：
- **文件存在** → 已有 Skill，切换到修改流程（第二节），不要重新创建。
- **文件不存在** → 继续创建流程。

### Step 3: 生成两件套

**V2 路径变更**：`_personal/{workspace_id}/{staff_id}/{skill_name}/`

#### 3a. 工具脚本

路径：`_personal/{workspace_id}/{staff_id}/{skill_name}/tool.py`
...
路径：`_personal/{workspace_id}/{staff_id}/{skill_name}/SKILL.md`
...
### Step 1: 定位 Skill
- 根据用户描述匹配 `_personal/{workspace_id}/{staff_id}/` 下的 Skill 目录
...
### Step 1: 确认目标
- 匹配 `_personal/{workspace_id}/{staff_id}/` 下的 Skill
...
1. 只能在 `_personal/{当前workspace_id}/{当前staff_id}/{skill_name}/` 下创建/修改/删除文件
2. 禁止读取或修改其他用户的文件
