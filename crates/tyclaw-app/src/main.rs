...
fn print_effective_config(
    ...
    if mode == "dingtalk" {
        ...
        println!("dingtalk.workspace_id: {}", dingtalk.workspace_id);
        println!("dingtalk.routes.count: {}", dingtalk.routes.len());
    }
...
async fn main() {
    ...
    let _api_key_from_cli_or_env = args.api_key.is_some();
